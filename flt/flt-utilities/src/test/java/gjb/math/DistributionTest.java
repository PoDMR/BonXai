...
public class DistributionTest extends TestCase {
    ...
    public void test_geometricDistribution() {
        final int NUMBER = 10000;
        final double P = 0.1;
        Map<Integer,Integer> count = new HashMap<Integer,Integer>();
        GeometricDistribution distr = new GeometricDistribution(P);
        for (int i = 0; i < NUMBER; i++) {
            int n = distr.getNext();
            if (!count.containsKey(n))
                count.put(n, 0);
        ...
        int i = 0;
        for (int n : count.keySet()) {
            assertTrue("right range " + n, 1 <= n);
            expected[i] = NUMBER*Math.pow(1.0 - P, n - 1.0)*P;
            observed[i] = count.get(n);
            i++;
        }
        try {
            assertTrue("chi^2 geometric",
                       !TestUtils.chiSquareTest(expected, observed, 0.05));
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
            fail("illegal argument");
    ...
    public void test_poissonDistribution() {
        final int NUMBER = 100000;
        final double MEAN = 10.0;
        Map<Integer,Integer> count = new HashMap<Integer,Integer>();
        PoissonDistribution distr = new PoissonDistribution(MEAN);
        for (int i = 0; i < NUMBER; i++) {
            int n = distr.getNext();
            if (!count.containsKey(n))
                count.put(n, 0);
        ...
        int i = 0;
        for (int n : count.keySet()) {
            assertTrue("right range " + n, 0 <= n);
            expected[i] = NUMBER*Math.pow(MEAN, n)*Math.exp(-MEAN)/MathUtils.factorialDouble(n);
            observed[i] = count.get(n);
            i++;
        }
        try {
            assertTrue("chi^2 poisson",
                       !TestUtils.chiSquareTest(expected, observed, 0.05));
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
            fail("illegal argument");
