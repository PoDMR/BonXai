...
import gjb.util.tree.iterators.BreadthFirstIterator;
import gjb.util.tree.iterators.LeafIterator;
...
public class Node implements java.io.Serializable {
	 ...
	 */
	public Node(Node node, Map<Node,Node> nodeMap) {
		super();
		this.init();
		this.key = node.key();
		this.value = node.value();
	 ...
	 * original node
	 */
	public Node deepClone(Map<Node,Node> nodeMap) {
		Node newNode = new Node(this, nodeMap);
		for (Iterator<Node> it = children(); it.hasNext(); ) {
			Node child = it.next();
	...
	Node modify(NodeModifier modifier, Map<String,Object> parameters)
	        throws NodeTransformException {
		Node newNode = modifier.modify(this, parameters);
		for (Iterator<Node> it = this.children(); it.hasNext(); ) {
			Node child = it.next();
	 ...
	 */
	public static Node parse(String str) throws SExpressionParseException {
		...
		if (nodeMatcher.matches()) {
			...
			if (childrenStr != null) {
				List<Node> children = Node.parseChildren(childrenStr);
	...
	protected static List<Node> parseChildren(String str)
		...
		for (int pos = 0; pos < str.length(); pos++) {
			...
			if (counter == 0) {
				children.add(Node.parse(str.substring(previousPos, pos + 1)));
				previousPos = pos + 1;
			}
		}
		if (counter != 0) {
    ...
    public boolean isDecendantOf(Node ancestor) {
        Node node = this;
    ...
    public static Node getLeastCommonAncestor(Node node1, Node node2) {
	 ...
	 */
	protected Tree setTree(Tree tree) {
		Tree oldTree = this.getTree();
		this.tree = tree;
		for (Iterator<Node> it = children(); it.hasNext(); ) {
	 ...
	 * specified
	 */
	public Node child(int index) throws IndexOutOfBoundsException {
	 ...
	 * operation was succesful, <code>false</code> otherwise
	 */
	public boolean addChildren(Collection<Node> newChildren) {
		boolean success = true;
		for (Iterator<Node> it = newChildren.iterator(); it.hasNext(); ) {
	...
	public Node setChild(int index, Node child)
		...
		Node oldChild = this.children.set(index, child);
		if (oldChild != null)
	...
	public Node removeChild(int index)
		throws IndexOutOfBoundsException {
		Node exChild = this.children.remove(index);
		if (exChild != null) {
