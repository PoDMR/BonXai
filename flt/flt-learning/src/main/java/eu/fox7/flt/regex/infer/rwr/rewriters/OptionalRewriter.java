 ...
 * 
 */
public class OptionalRewriter implements AutomatonRewriter {
    ...
    public Automaton rewrite(Automaton automaton, int[] indices) {
        ...
        newAutomaton.set(automaton.getNumberOfStates() - 1, automaton.getNumberOfStates() - 1, 0);
        for (int k = 0; k < automaton.getNumberOfStates(); k++)
            if (k != i)
    ...
    protected String makeLabel(Automaton automaton, int i) {
        return Regex.LEFT_BRACKET + Regex.ZERO_OR_ONE_OPERATOR + " " +
