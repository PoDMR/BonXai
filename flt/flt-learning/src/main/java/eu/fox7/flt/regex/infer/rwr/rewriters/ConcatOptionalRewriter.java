 ...
 * 
 */
public class ConcatOptionalRewriter extends ConcatRewriter {

    @Override
    protected String makeLabel(Automaton automaton, int i, int j) {
        return Regex.LEFT_BRACKET + Regex.CONCAT_OPERATOR + " " +
            automaton.getLabel(i) + " " + Regex.LEFT_BRACKET +
            Regex.ZERO_OR_ONE_OPERATOR + " " + automaton.getLabel(j) +
            Regex.RIGHT_BRACKET + Regex.RIGHT_BRACKET;
    }
