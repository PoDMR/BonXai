...
public class XSDUnion implements SchemaOperation {
	private Collection<XSDSchema> schemas = new LinkedList<XSDSchema>();

	public XSDSchema computeUnion(XSDSchema...schemas) throws SchemaToolkitException {
		...
		TypeAutomatonUnion taUnion = new TypeAutomatonUnion();
		TypeAutomaton resultTA = taUnion.apply(typeAutomata);
	...
	public SchemaHandler apply() throws SchemaToolkitException {
		return this.computeUnion(schemas.toArray(new XSDSchema[0])).getSchemaHandler();
	}
