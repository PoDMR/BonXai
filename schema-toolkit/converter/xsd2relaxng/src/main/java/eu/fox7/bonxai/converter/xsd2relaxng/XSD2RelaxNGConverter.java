...
import eu.fox7.schematoolkit.xsd.om.parser.ForeignSchemaLoader;
 ...
 * @author Lars Schmidt
 */
public class XSD2RelaxNGConverter extends ConverterBase implements SchemaConverter{
    /**
...
//        this.loadPreferences();
     ...
     */
    @Override
    public RelaxNGSchema convert(Schema schema) throws ConversionFailedException {
		if (!(schema instanceof XSDSchema))
			throw new ConversionFailedException("Only conversion from XML SChema supported.");
    	this.init();
		this.xmlSchema = (XSDSchema) schema;
...
//    			StatusLogger.logInfo("XSD2RNG", "Possible substitutions has been found for " + typeInheritanceInformation.size() + " types");
    		 ...
    		 */
    		LinkedHashMap<eu.fox7.schematoolkit.xsd.om.Element, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Element>> elementInheritanceInformation = null;
    		if (HANDLE_SUBSTITUTIONS) {
...
//    			StatusLogger.logInfo("XSD2RNG", "Possible substitutions has been found for " + typeInheritanceInformation.size() + " elements");
    			...
    			elementInheritanceInformation = new LinkedHashMap<eu.fox7.schematoolkit.xsd.om.Element, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Element>>();
    		}
    		/**
    		 * 4.
...
//    		StatusLogger.logInfo("XSD2RNG", "Create a valid root pattern");
    		...
    		grammar = new Grammar(RELAXNG_XMLSCHEMA_DATATYPE_NAMESPACE, defaultNamespace, RelaxNGSchema.RELAXNG_NAMESPACE);
    		...
    		if (!xmlSchema.getNamespaces().isEmpty()) {
    			for (IdentifiedNamespace identifiedNamespace: xmlSchema.getNamespaces()) {
    				if (identifiedNamespace.getIdentifier() != null && !identifiedNamespace.getIdentifier().equals("")) {
    					grammar.getNamespaceList().addNamespace(new IdentifiedNamespace(identifiedNamespace.getIdentifier(), identifiedNamespace.getUri()));
    				}
    			}
    		}
...
//    		StatusLogger.logLastInfoMessage("XSD2RNG", "Conversion completed: RELAX NG schema created");
     ...
     */
    private void findNotUsedComponents(XSDSchema xmlSchema, LinkedHashSet<XSDSchema> alreadySeenSchemas, LinkedHashSet<Type> foundTypes, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Attribute> foundAttributes, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.AttributeGroup> foundAttributeGroups, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Group> foundGroups) {
        ...
        if (!alreadySeenSchemas.contains(xmlSchema)) {
            ...
            // Process Groups
            for (Iterator<eu.fox7.schematoolkit.xsd.om.Group> it = xmlSchema.getGroups().iterator(); it.hasNext();) {
                eu.fox7.schematoolkit.xsd.om.Group group = it.next();
     ...
     */
    private void generateDefinesForNotUsedComponents(LinkedHashMap<Type, LinkedHashSet<Type>> typeInheritanceInformation, LinkedHashMap<eu.fox7.schematoolkit.xsd.om.Element, LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Element>> elementInheritanceInformation) {

        // Provide sets for each type of a possible unused schema component
        LinkedHashSet<Type> foundTypes = new LinkedHashSet<Type>();
        LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Attribute> foundAttributes = new LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Attribute>();
        LinkedHashSet<eu.fox7.schematoolkit.xsd.om.AttributeGroup> foundAttributeGroups = new LinkedHashSet<AttributeGroup>();
        LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Group> foundGroups = new LinkedHashSet<eu.fox7.schematoolkit.xsd.om.Group>();
...
//            StatusLogger.logInfo("XSD2RNG", "Found " + foundAttributes.size() + " attributes, which are defined but not used in the XML XSDSchema document");
            ...
            for (Iterator<eu.fox7.schematoolkit.xsd.om.Attribute> it = foundAttributes.iterator(); it.hasNext();) {
                eu.fox7.schematoolkit.xsd.om.Attribute attribute = it.next();
...
//            StatusLogger.logInfo("XSD2RNG", "Found " + foundAttributeGroups.size() + " attribute groups, which are defined but not used in the XML XSDSchema document");
            ...
            for (Iterator<eu.fox7.schematoolkit.xsd.om.AttributeGroup> it = foundAttributeGroups.iterator(); it.hasNext();) {
                eu.fox7.schematoolkit.xsd.om.AttributeGroup attributeGroup = it.next();
...
//            StatusLogger.logInfo("XSD2RNG", "Found " + foundGroups.size() + " groups, which are defined but not used in the XML XSDSchema document");
            // Handle found groups
            for (Iterator<eu.fox7.schematoolkit.xsd.om.Group> it = foundGroups.iterator(); it.hasNext();) {
                eu.fox7.schematoolkit.xsd.om.Group group = it.next();
...
//            StatusLogger.logInfo("XSD2RNG", "Found " + foundTypes.size() + " types, which are defined but not used in the XML XSDSchema document");
            // Handle found types
            for (Iterator<eu.fox7.schematoolkit.xsd.om.Type> it = foundTypes.iterator(); it.hasNext();) {
