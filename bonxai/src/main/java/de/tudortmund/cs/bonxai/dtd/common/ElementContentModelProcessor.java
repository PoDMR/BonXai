...
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelCountingPatternIllegalMinValueException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelCountingPatternNotAllowedDTDValueException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelEmptyChildParticleListException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelIllegalChildParticleException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelIllegalMixedDuplicateElementException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelIllegalMixedParticleException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelIllegalParticleException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelIllegalStringForMixedContentException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelNullParticleException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelStringEmptyException;
import de.tudortmund.cs.bonxai.dtd.common.exceptions.ContentModelStringTokenizerIllegalStateException;
...
public class ElementContentModelProcessor {
    ...
    private Particle convertTokenToCorrespondingParticle(ElementContentModelStringTokenizer tokenizer) throws Exception {
        ...
        do {
            ...
            if (currentChar == '#') {
                ...
                ElementRef elementRef = new ElementRef(dtd.registerElementFromContentModel(element));
                particle = elementRef;
                if (this.mixed){ 
                	for (Particle currentChild : childParticleList) {
                		if (currentChild instanceof ElementRef) {
                			if (((ElementRef) currentChild).getElement().getName().equals(element.getName())) {
                ...
                if (currentChar == '?') {
                    ...
                    countingPattern.addParticle(particle);
    ...
    public String convertParticleToRegExpString(Element element) throws Exception {
        ...
        if (element.hasAnyType()) {
            ...
            // The countingPattern must contain only one particle
            if (countingPattern.getParticles().size() != 1) {
                throw new ContentModelIllegalMixedParticleException("Element \"" + element.getName() + "\": CountingPatternSize: " + Integer.toString(countingPattern.getParticles().size()) + ", but has to be 1.");
            }
            ...
            if (!(countingPattern.getParticles().getFirst() instanceof ChoicePattern) && !(countingPattern.getParticles().getFirst() instanceof ElementRef)) {
                throw new ContentModelIllegalMixedParticleException("Element \"" + element.getName() + "\": CountingPattern child instance: " + countingPattern.getParticles().getFirst().getClass().getName());
            }

            if (countingPattern.getParticles().getFirst() instanceof ChoicePattern) {
                // Choicepattern
                ...
                for (Particle currentParticle : choicePattern.getParticles()) {
                    if (currentParticle instanceof ElementRef) {
                        ...
                        Element currentElement = elementRef.getElement();
                        String currentElementName = currentElement.getName();

                        if (!nameChecker.checkForXMLName(currentElementName)) {
                            throw new IllegalNAMEStringException("Element: " + currentElementName, currentElementName);
                        }

                        if (!elementNames.contains(currentElementName)) {
                            elementNames.add(currentElementName);
                ...
                // Case: ElementRef
                if (countingPattern.getParticles().getFirst() instanceof ElementRef) {
                    ...
                    if (!nameChecker.checkForXMLName(currentElement.getName())) {
                        throw new IllegalNAMEStringException("Element: " + currentElement.getName(), currentElement.getName());
                    }
                    mixedParticleString += currentElement.getName() + "|";
                }
            }
     ...
     * @throws Exception
     */
    private String convertParticleToRegExpString(Particle particle, String elementName) throws Exception {
        DTDNameChecker nameChecker = new DTDNameChecker();
        String currentRegExpString = "";
        ...
        if (particle instanceof AnyPattern) {
            ...
            if (countingPattern == null || countingPattern.getParticles().isEmpty()) {
                throw new ContentModelEmptyChildParticleListException("countingPattern");
            }

            String countingPatternType = "";

            if (countingPattern.getMin() == null) {
                throw new ContentModelCountingPatternIllegalMinValueException("null");
            } else if (countingPattern.getMin() == 0 && countingPattern.getMax() == null) {
                countingPatternType = "*";
            ...
            if (countingPattern.getParticles().size() == 1 && ((countingPattern.getParticles().getFirst() instanceof CountingPattern) || (countingPattern.getParticles().getFirst() instanceof ElementRef))) {
                currentRegExpString += "(" + convertParticleToRegExpString(countingPattern.getParticles().getFirst(), elementName) + ")" + countingPatternType;
            } else {
                currentRegExpString += convertParticleToRegExpString(countingPattern.getParticles().getFirst(), elementName) + countingPatternType;
            }
            ...
            if (!nameChecker.checkForXMLName(elementRef.getElement().getName())) {
                throw new IllegalNAMEStringException("Element: " + elementRef.getElement().getName(), elementRef.getElement().getName());
            }
            currentRegExpString += elementRef.getElement().getName();
        } else {
            if (particle == null) {
                // Particle is null
                throw new ContentModelNullParticleException(elementName, "Particle is null");
            } else {
                // Particle is of illegal type
                throw new ContentModelIllegalParticleException(elementName, particle.getClass().getName());
            }
        }
