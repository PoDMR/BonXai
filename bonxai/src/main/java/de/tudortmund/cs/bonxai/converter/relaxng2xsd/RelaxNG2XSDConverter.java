...
import de.tudortmund.cs.bonxai.relaxng.tools.*;
 ...
 * RELAX NG object model (de.tudortmund.cs.bonxai.relaxng) to the
 * XSD object model (de.tudortmund.cs.bonxai.xsd).
 *
...
public class RelaxNG2XSDConverter extends ConverterBase {
     ...
     * 
     */
    public static boolean RECURSION_MODE_COMPLEXTYPE = true;
    /**
    ...
    public XSDSchema convert() throws ConversionFailedException {
    	try {
    		...
    		if (this.relaxng.getRootPattern().getAttributeNamespace() != null) {
    			this.xmlSchema.setTargetNamespace(this.relaxng.getRootPattern().getAttributeNamespace());
    		}
    			...
    			for (Iterator<ForeignSchema> it1 = this.xmlSchema.getForeignSchemas().iterator(); it1.hasNext();) {
    				...
    				if (!this.xmlSchema.getNamespaceList().getIdentifiedNamespaces().isEmpty()) {
    					for (Iterator<IdentifiedNamespace> it = this.xmlSchema.getNamespaceList().getIdentifiedNamespaces().iterator(); it.hasNext();) {
    						...
    						foreignSchema.getSchema().getNamespaceList().addIdentifiedNamespace(identifiedNamespace);
    					}
    				}
    			}
    		}
    		...
    		// Case "Type":
    			for (Iterator<SymbolTableRef<de.tudortmund.cs.bonxai.xsd.Type>> it = this.xmlSchema.getTypeSymbolTable().getReferences().iterator(); it.hasNext();) {
    				SymbolTableRef<de.tudortmund.cs.bonxai.xsd.Type> symbolTableRefType = it.next();
    				if (!symbolTableRefType.getReference().getNamespace().equals(this.xmlSchema.getTargetNamespace())) {
    					...
    					if (importedSchema != null) {
    						SymbolTable<de.tudortmund.cs.bonxai.xsd.Type> typeSymbolTable = (SymbolTable<de.tudortmund.cs.bonxai.xsd.Type>) importedSchema.getSchema().getTypeSymbolTable();
    						typeSymbolTable.setReference(symbolTableRefType.getKey(), symbolTableRefType);
    						importedSchema.getSchema().addType(symbolTableRefType);
    					}
    				}
    				for (Iterator<ForeignSchema> it1 = this.xmlSchema.getForeignSchemas().iterator(); it1.hasNext();) {
    					ForeignSchema foreignSchema = (ImportedSchema) it1.next();
    					if (foreignSchema instanceof ImportedSchema) {
    						...
    						if (symbolTableRefType.getReference().getNamespace().equals(RelaxNG2XSDConverter.XMLSCHEMA_NAMESPACE)) {
    							SymbolTable<de.tudortmund.cs.bonxai.xsd.Type> typeSymbolTable = (SymbolTable<de.tudortmund.cs.bonxai.xsd.Type>) importedSchema.getSchema().getTypeSymbolTable();
    							typeSymbolTable.setReference(symbolTableRefType.getKey(), symbolTableRefType);
    						}
    					}
    ...
    private void createIdentifiedNamespaces(HashMap<String, String> identifiedNamespaces) {
        this.xmlSchema.getNamespaceList().addIdentifiedNamespace(new IdentifiedNamespace("xs", RelaxNG2XSDConverter.XMLSCHEMA_NAMESPACE));
        ...
        for (Iterator<String> it = identifiedNamespaces.keySet().iterator(); it.hasNext();) {
            ...
            if (this.xmlSchema.getNamespaceList().getNamespaceByIdentifier(abbreviation).getUri() != null && !this.xmlSchema.getNamespaceList().getNamespaceByIdentifier(abbreviation).getUri().equals(uri)) {
                this.xmlSchema.getNamespaceList().addIdentifiedNamespace(new IdentifiedNamespace(abbreviation + "_" + this.xmlSchema.getNamespaceList().getIdentifiedNamespaces().size(), uri));
            } else {
                this.xmlSchema.getNamespaceList().addIdentifiedNamespace(new IdentifiedNamespace(abbreviation, uri));
            }
        }
    }
