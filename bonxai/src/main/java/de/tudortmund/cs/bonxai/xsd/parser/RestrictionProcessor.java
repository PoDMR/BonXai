...
public class RestrictionProcessor extends Processor {
    ...
    private SimpleType anonymousSimpleType;
            ...
            switch (CASE.valueOf(nodeName.toUpperCase())) {
                ...
                case SIMPLETYPE:
                    if (grandParentName.toUpperCase().equals(restrictionType.SIMPLETYPE.name())
                        ...
                        this.anonymousSimpleType = (SimpleType) simpleTypeProcessor.processNode(childNode).getReference();
                    } else {
    ...
    protected Inheritance processNode(Node node) throws Exception {
        ...
        String fullQualifiedBaseTypeName = null;
        String baseTypeName = null;
        SymbolTableRef<Type> baseTypeRef = null;
        ...
        if (node.getAttributes() != null && node.getAttributes().getNamedItem("base") != null) {
            baseTypeName = node.getAttributes().getNamedItem("base").getNodeValue();

            if (baseTypeName.equals("")) {
                throw new EmptyInheritanceBaseException("restriction");
            }
            if (!isQName(baseTypeName)) {
                throw new InvalidQNameException(baseTypeName, "restriction");
            }
            fullQualifiedBaseTypeName = getName(baseTypeName);
        ...
        if (parentNodeName.toUpperCase().equals(restrictionType.SIMPLETYPE.name())
            ...
            if (parentNodeName.toUpperCase().equals(restrictionType.SIMPLETYPE.name())) {
                SymbolTableRef<Type> resultingTypeRef = null;
                if (baseTypeRef != null) {
                    resultingTypeRef = baseTypeRef;
                } else {
                    resultingTypeRef = null;
                }
                this.inheritance = new SimpleContentRestriction(resultingTypeRef);
            } else {
                this.inheritance = new SimpleContentRestriction(baseTypeRef);
            }

            this.enumeration = new LinkedList<String>();

        } else if (parentNodeName.toUpperCase().equals(restrictionType.COMPLEXCONTENT.name())) {
            this.inheritance = new ComplexContentRestriction(baseTypeRef);
        }
        ...
        if (parentNodeName.toUpperCase().equals(restrictionType.SIMPLETYPE.name())
            ...
            if (parentNodeName.toUpperCase().equals(restrictionType.SIMPLETYPE.name())) {
                if (baseTypeRef != null && this.anonymousSimpleType != null) {
                    throw new MultipleTypesException(restrictionType.SIMPLETYPE.name(), "restriction");
                }
                if (((SimpleContentRestriction) this.inheritance).getBase() == null) {
                    ((SimpleContentRestriction) this.inheritance).setBase(this.schema.getTypeSymbolTable().getReference(anonymousSimpleType.getName()));
                }
            } else {
                ((SimpleContentRestriction) this.inheritance).setAnonymousSimpleType(anonymousSimpleType);
            }
