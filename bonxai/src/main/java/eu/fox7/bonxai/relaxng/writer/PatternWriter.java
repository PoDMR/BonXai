...
import eu.fox7.bonxai.relaxng.writer.exceptions.EmptyPatternException;
import eu.fox7.bonxai.relaxng.writer.exceptions.InvalidAnyUriException;
import eu.fox7.bonxai.relaxng.writer.exceptions.InvalidHrefException;
import eu.fox7.bonxai.relaxng.writer.exceptions.InvalidNCNameException;
import eu.fox7.bonxai.relaxng.writer.exceptions.InvalidQNameException;
import eu.fox7.bonxai.relaxng.writer.exceptions.MissingHrefException;
import eu.fox7.bonxai.relaxng.writer.exceptions.MissingNameException;
import eu.fox7.bonxai.relaxng.writer.exceptions.MissingPatternException;
import eu.fox7.bonxai.relaxng.writer.exceptions.MultipleNameException;
import eu.fox7.bonxai.relaxng.writer.exceptions.PatternReferencedButNotDefinedException;
import eu.fox7.bonxai.relaxng.writer.exceptions.UnknownPatternException;
    ...
    private org.w3c.dom.Element createNodeForGrammar(Grammar grammar) throws Exception {
        ...
        if (grammar.getDefinedPatternNames() != null && !grammar.getDefinedPatternNames().isEmpty()) {
            for (Iterator<String> itGrammarDefine = grammar.getDefinedPatternNames().iterator(); itGrammarDefine.hasNext();) {
                ...
                LinkedList<Define> defineList = grammar.getDefinedPatternsFromLookUpTable(defineName);
...
//            }
        ...
        if (ref.getDefineList() != null && !ref.getDefineList().isEmpty()) {
            if (!isNCName(ref.getDefineList().getFirst().getName())) {
                throw new InvalidNCNameException(ref.getDefineList().getFirst().getName(), "name of ref");
            }
            refNode.setAttribute("name", ref.getDefineList().getFirst().getName());
        } else {
            // Define is not set!
    ...
    private org.w3c.dom.Element createNodeForParentRef(ParentRef parentRef) throws Exception {
        ...
        if (parentRef.getDefineList() != null && !parentRef.getDefineList().isEmpty()) {
            if (parentRef.getDefineList() == null || !isNCName(parentRef.getDefineList().getFirst().getName())) {
                throw new InvalidNCNameException(parentRef.getDefineList().getFirst().getName(), "name of parentRef");
            }
            parentRefNode.setAttribute("name", parentRef.getDefineList().getFirst().getName());
        }  else {
            // Define is not set!
