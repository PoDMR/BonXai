    ...
    protected XSDSchema processNode(Node node) throws Exception {
        ...
        if (attributes != null) {
            for (int i = 0; i < node.getAttributes().getLength(); i++) {
                if (((Node) node.getAttributes().item(i)).getNodeName().startsWith("xmlns")) {
                    ...
                    if (currentNode.getNodeName().equals("xmlns")) {
                        schema.setTargetNamespace(currentNode.getNodeValue());
                    } else {
                        ...
                        schema.getNamespaceList().addIdentifiedNamespace(new IdentifiedNamespace(currentNode.getLocalName(), currentNode.getNodeValue()));
                    }
                }
            }
            ...
            if (attributes.getNamedItem("targetNamespace") != null && !((Attr) attributes.getNamedItem("targetNamespace")).getNodeValue().equals("")) {
                ...
                schema.getNamespaceList().addIdentifiedNamespace(new IdentifiedNamespace("xml", "http://www.w3.org/XML/1998/namespace"));
            } else {
        ...
        for (int i = 0; i < schema.getElements().size(); i++) {
            for (int j = i + 1; j < schema.getElements().size(); j++) {
                if (schema.getElements().get(i).getName().equals(schema.getElements().get(j).getName())) {
                    throw new DuplicateElementException(schema.getElements().get(i).getName());
                }
            }
        }
        for (int i = 0; i < schema.getAttributes().size(); i++) {
            for (int j = i + 1; j < schema.getAttributes().size(); j++) {
                if (schema.getAttributes().get(i).getName().equals(schema.getAttributes().get(j).getName())) {
                    throw new DuplicateAttributeException(schema.getAttributes().get(i).getName());
                }
            }
        }
        return schema;
            ...
            switch (CASE.valueOf(nodeName.toUpperCase())) {
                ...
                case ATTRIBUTE:
                    ...
                    if (object instanceof Attribute) {
                        ...
                        schema.getAttributeSymbolTable().updateOrCreateReference(attribute.getName(), attribute);
                        schema.addAttribute(schema.getAttributeSymbolTable().getReference(attribute.getName()));
                    } else {
                ...
                case ELEMENT:
                    ...
                    if (particle instanceof Element) {
                        ...
                        schema.getElementSymbolTable().updateOrCreateReference(elementName, element);
                        schema.addElement(schema.getElementSymbolTable().getReference(elementName));
                    } else {
                ...
                case COMPLEXTYPE:
                    ...
                    schema.addType(complexTypeProcessor.processNode(childNode));
                    break;
                case SIMPLETYPE:
                    if (getDebug()) {
                    ...
                    schema.addType(simpleTypeProcessor.processNode(childNode));
                    break;
                case GROUP:
                    if (getDebug()) {
                    ...
                    if (object instanceof Group) {
                        ...
                        schema.addGroup(schema.getGroupSymbolTable().getReference(group.getName()));
                    } else {
                        throw new IllegalObjectReturnedException(object.getClass().getName(), "schema element");
                    }
                    break;
                case ATTRIBUTEGROUP:
                    ...
                    if (attributeParticle instanceof AttributeGroup) {
                        ...
                        schema.addAttributeGroup(schema.getAttributeGroupSymbolTable().getReference(attributeGroup.getName()));
                    } else {
                        throw new IllegalObjectReturnedException(attributeParticle.getClass().getName(), "schema element");
                    }
                    break;
                case ANNOTATION:
                    if (getDebug()) {
