...
public class KeyrefProcessor extends Processor {
    ...
    protected KeyRef processNode(Node node) throws Exception {
        ...
        if (schema.getKeyAndUniqueSymbolTable().hasReference(refer)) {
            keyRef = new KeyRef(keyRefName, "", schema.getKeyAndUniqueSymbolTable().getReference(refer));
        } else {
            Key keyElement = new Key(refer, "");
            keyElement.setDummy(true);
            keyRef = new KeyRef(keyRefName, "", schema.getKeyAndUniqueSymbolTable().updateOrCreateReference(refer, keyElement));
        }
        visitChildren(node);
