...
public class ElementProcessor extends Processor {
    ...
    protected Particle processNode(Node node) throws Exception {
        ...
        element = new Element(getNamespace(node), getLocalName(node));
        ...
        if (attributes != null) {
            ...
            // The substitutionGroup head element like this element have to be global elements and the type of this element must be derived from the head element
            if (attributes.getNamedItem("substitutionGroup") != null) {
                ...
                String substitutionGroupName = getName(((Attr) attributes.getNamedItem("substitutionGroup")).getValue());
            ...
            if (attributes.getNamedItem("type") != null) {
                ...
                if (element.getType() != null) {
                    throw new MultipleTypesException("element", element.getName());
                }
                String typeName = getName(((Attr) attributes.getNamedItem("type")).getValue());
                if (!schema.getTypeSymbolTable().hasReference(typeName)) {
                    SimpleType simpleType = new SimpleType(typeName, null);
                    simpleType.setDummy(true);
                    element.setType(schema.getTypeSymbolTable().updateOrCreateReference(typeName, simpleType));
                } else {
                    element.setType(this.schema.getTypeSymbolTable().getReference(typeName));
                }
            }
            if (attributes.getNamedItem("ref") != null) {
                ...
                if (!schema.getElementSymbolTable().hasReference(refName)) {
                    Element elementRefElement = new Element(refName);
                    elementRefElement.setDummy(true);
                    schema.getElementSymbolTable().updateOrCreateReference(refName, elementRefElement);
                }
                ElementRef elementRef = new ElementRef(schema.getElementSymbolTable().getReference(refName));
                if (attributes.getNamedItem("id") != null) {
        ...
        if (element.getType() == null) {
            ...
            element.setType(schema.getTypeSymbolTable().updateOrCreateReference(anyTypeName, new SimpleType(anyTypeName, null)));
        }
        if (maxOccursValue == null || (minOccursValue != 1 || maxOccursValue != 1)) {
            CountingPattern countingPattern = new CountingPattern(minOccursValue, maxOccursValue);
            ...
            switch (CASE.valueOf(nodeName.toUpperCase())) {
                case COMPLEXTYPE:
                    ...
                    if (element.getType() != null) {
                        ...
                        element.setType(complexTypeProcessor.processNode(childNode));
                    }
                    break;
                case SIMPLETYPE:
                    if (getDebug()) {
                    ...
                    if (element.getType() != null) {
                        ...
                        element.setType(simpleTypeProcessor.processNode(childNode));
                    }
                    break;
                case UNIQUE:
                    if (getDebug()) {
