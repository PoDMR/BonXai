...
/**
 * Test of class InterleaveProcessor
 * @author Lars Schmidt
 */
public class InterleaveProcessorTest extends junit.framework.TestCase {
    ...
    /**
     * Test valid case of class InterleaveProcessor.
     * @throws Exception
     */
    @Test
    public void testValidCase() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/interleaveTests/interleave_valid.rng"));
        ...
        assertTrue(grammar.getDefineLookUpTable().getReference("define").getReference().getFirst().getPatterns().getFirst() instanceof Interleave);

        Interleave interleave = (Interleave) grammar.getDefineLookUpTable().getReference("define").getReference().getFirst().getPatterns().getFirst();
        LinkedList<Pattern> temp = interleave.getPatterns();

        Iterator<Pattern> it = interleave.getPatterns().iterator();
    ...
    /**
     * Test invalid case of class InterleaveProcessor, with UnsupportedContentException.
     * @throws Exception
     */
    @Test
    public void testInValidCaseWithUnsupportedContentException() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/interleaveTests/interleave_invalid_content.rng"));
        } catch (UnsupportedContentException error) {
            return;
        }
        fail("Unsupported content under interleave-tag was not detected.");
    }

    /**
     * Test invalid case of class InterleaveProcessor, with EmptyPatternException.
     * @throws Exception
     */
    @Test
    public void testInValidCaseWithEmptyPatternException() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/interleaveTests/interleave_empty_content.rng"));
        } catch (EmptyPatternException error) {
            return;
        }
        fail("There is no content under a interleave-tag, but this was not detected.");
    }
}
