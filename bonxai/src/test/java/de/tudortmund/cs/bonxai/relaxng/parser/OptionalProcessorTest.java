...
/**
 * Test of class OptionalProcessor
 * @author Lars Schmidt
 */
public class OptionalProcessorTest extends junit.framework.TestCase {
    ...
    /**
     * Test valid case of class OptionalProcessor.
     * @throws Exception
     */
    @Test
    public void testValidCase() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/optionalTests/optional_valid.rng"));
        ...
        assertTrue(grammar.getDefineLookUpTable().getReference("define").getReference().getFirst().getPatterns().getFirst() instanceof Optional);

        Optional optional = (Optional) grammar.getDefineLookUpTable().getReference("define").getReference().getFirst().getPatterns().getFirst();
        LinkedList<Pattern> temp = optional.getPatterns();

        Iterator<Pattern> it = optional.getPatterns().iterator();
    ...
    /**
     * Test invalid case of class OptionalProcessor, with UnsupportedContentException.
     * @throws Exception
     */
    @Test
    public void testInValidCaseWithUnsupportedContentException() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/optionalTests/optional_invalid_content.rng"));
        } catch (UnsupportedContentException error) {
            return;
        }
        fail("Unsupported content under optional-tag was not detected.");
    }

    /**
     * Test invalid case of class OptionalProcessor, with EmptyPatternException.
     * @throws Exception
     */
    @Test
    public void testInValidCaseWithEmptyPatternException() throws Exception {
        try {
            rngRootProcessor.processNode(RNGUtilities.getSchemaNode("tests/de/tudortmund/cs/bonxai/relaxng/parser/rngs/optionalTests/optional_empty_content.rng"));
        } catch (EmptyPatternException error) {
            return;
        }
        fail("There is no content under a optional-tag, but this was not detected.");
    }
}
